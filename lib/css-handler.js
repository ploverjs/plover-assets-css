...
const logger = require('plover-logger')('plover-assets-css');



module.exports = PloverAssetsHandlerCss;


function* PloverAssetsHandlerCss(path, source, info, options) {
  ...
  const enableConcat = settings.assets && settings.assets.enableConcat;
  logger.info('build %s', path);
  const opts = getOptions(settings, info, path);
  return yield new CleanCSS(opts).minify(source).then(o => {
    return enableConcat ? filterCss(path, o.styles, info, options) : o.styles;
  });
}
...
function getOptions(settings, info, path) {
  const opts = {};
  Object.assign(opts, (settings.assets || {}).css);
